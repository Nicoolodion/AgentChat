...
> ## Integration Notes (Chatinterface) — overrides Kimi defaults
> This skill was ported from the Kimi agent. A few Kimi-specific conventions DO NOT apply here; follow this section instead:
> 1. **Deliver .pptx, not .pptd alone.** Unlike Kimi, our frontend has no .pptd editor/export button, so a `.pptd` by itself is not a usable deliverable. After authoring + validating the PPTD project, ALWAYS render it to `.pptx` with the `pptx_render` tool (`input_path` = the `.pptd`, `output_path` = `output/<name>.pptx`) so the user gets a downloadable file. Keep the `.pptd` project in place alongside it.
> 2. **Tool names.** The rest of this skill and the guideline/format docs mention `read_file`/`write_file`/`edit_file`. In this agent the equivalents are `file_read` and `file_write`. There is no `edit_file` tool — to change part of a file, rewrite the whole file with `file_write` (read it with `file_read` first if needed).
> 3. **Scripts → dedicated tools.** Do NOT invoke `scripts/check.sh`, `scripts/convert.sh`, or `scripts/screenshot.sh` through the `shell` tool. Use these tools instead:
>    - `pptx_check(input_path)` — the `check` step (validate a `.pptd`).
>    - `pptx_render(input_path, output_path)` — the `convert` step (`.pptd` → `.pptx` to deliver, or `.pptx` → `.pptd` to read/edit an upload).
>    - `pptx_screenshot(input_path, output_path, pages?)` — the `screenshot` step (render `.pptx`/`.pptd` pages to PNGs in an output directory).
> 4. **In-place delivery still applies:** keep the `.pptd`, its `pages/`, and `images/` together in one project directory; do not move the `.pptd` alone.
> 5. **Other rules below (file isolation, workflow, parallel writes, checker fix loop, quality standards) apply as written.**
